...
class MySQLProvider extends SettingProvider {
	...
	async init(client) {
		this.client = client;
		await this.db.execute('CREATE TABLE IF NOT EXISTS `ServerSettings` (`GuildID` VARCHAR(20) NOT NULL, `Config` TEXT NOT NULL, `WelcomeSettings` VARCHAR(20) NOT NULL, PRIMARY KEY (`GuildID`))');

		// Load all settings
		const rows = await this.db.execute('SELECT GuildID, Config FROM ServerSettings').then((res) => res[0]);
		for(const row of rows) {
			let settings;
			try {
				settings = JSON.parse(row.Config);
			} catch(err) {
				client.emit('warn', `MySQLProvider couldn't parse the settings stored for guild ${row.GuildID}.`);
				continue;
			}
			...
			if(guild !== 'global' && !client.guilds.cache.has(row.GuildID)) {
                continue;
            }
	...
	async set(guild, key, val) {
        ...
        await this.db.execute('REPLACE INTO ServerSettings VALUES(?, ?, ?)', [ guild !== 'global' ? guild : 0, JSON.stringify(settings), "NaN" ]);
		if(guild === 'global') {
	...
	async clear(guild) {
        ...
        await this.db.execute('DELETE FROM ServerSettings WHERE guild = ?', [ (guild !== 'global' ? guild : 0) ]);
	}
